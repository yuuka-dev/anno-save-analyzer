...
- :class:`FactoryRecipeTable` で building_guid → recipe を引き，出力物資 /
  入力物資 / tpmin を取得．未登録 building でも行は表示し，名称は
  ``Building_<guid>`` のような fallback を使う．recipe が無い行では出力
  ``—`` / 入力 空 / 生産レート ``0.00`` の recipe-less 表示にする．
...
class ProductionOverviewScreen(Screen):
    ...
    # Tree node の data に持たせる selector．root は ``None``．非 root は
    # ``(kind, value)`` tuple で， ``kind`` は ``"session"`` または ``"island"``，
    # ``value`` はそれぞれ session_id (sid) / island_key を表す．
    ...
    def compose(self) -> ComposeResult:
        # border_title / Static 初期値はすべてここで widget 作成時に確定させる．
        # ``set_localizer`` → ``refresh(recompose=True)`` で compose が再実行
        # されると最新 locale の文言で再構築される．on_mount は recompose 時に
        # 再呼出されない仕様のため，文言設定をここに集約することで stale 表示
        # を防ぐ．
        yield Header(show_clock=False)
        tree = self._build_tree()
        tree.border_title = self._loc("production.tree.title")
        # 初期 summary は純粋関数で再計算 (table は _build_table 内で同じ集計済)．
        ...
        # All タブ初期はレート集計を summary に出さないため rate=0 で OK．
        initial_summary = self._format_summary(islands, total_factories, 0.0)
        table_pane = Vertical(
            Static(initial_summary, id="production-summary"),
            self._build_table(),
            id="production-table-pane",
        )
        table_pane.border_title = self._loc("production.table.title")
        detail_pane = Vertical(
            Static(self._loc("production.detail.empty"), id="production-detail"),
