...
> **状態**: 草案 (revision 4、Q-3〜Q-11 確定，nano 互換ホットキー採用)
> **目標出荷**: 兼業 2-3 週間
...
| **Q-6 (確定)** | items.yaml の v0.3.0 スコープと言語 | **英日併記必須**．上位 20 物資を pre-populate，残りは `scaffold-items` ヘルパで |
...
| ID | 内容 | タイミング |
|---|---|---|
| W-1 | `<TradedGoods>` の親タグ確認（`<RouteMap>` vs `<ActiveTradeHandler>` vs その他）．`sample_anno117.a8s` Session 0 で 10 分の hex dump | Week 1 実装開始直前 |
