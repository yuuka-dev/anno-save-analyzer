...
> **状態**: 草案 (revision 3、Anno 1800 統計画面の参考スクショを反映)
> **目標出荷**: 兼業 2-3 週間
...
**参考**: レイアウトは **Anno 1800 の「統計」画面**を踏襲（3 カラム構成: 左にセッション×島ツリー / 中央に物資テーブル / 右にコンテキストペイン．右は取引相手別と物資の時系列グラフを切替）．参考スクショは `Pictures/`（gitignore 済，著作権配慮でリポジトリには入れない）にローカル保管．
...
|---|---|
| `q` | 戻る / 終了 |
| `t` | trade statistics 画面 |
| `r` | route detail 画面 |
| `d` | snapshot diff 画面（S-1） |
| `g` | 右ペインのトグル（partners ↔ chart, S-7） |
| `[` / `]` | フォーカスペインの縮小 / 拡大 |
| `/` | 中央表のフィルタ |
| `s` | ソート列切替 |
| `gg` / `G` | 先頭 / 末尾 |
...
| **Q-9** | Anno 117 の `SECONDS_PER_TICK`．0.05 秒（20Hz）で確定？ | week 1 中に 2 セーブを N 分間隔で取って tick 差分から実測 |
| **Q-10 (新規)** | 折れ線グラフのライブラリ選定: `textual-plotext`（plotext を Textual ウィジェット化したもの）vs Unicode 手描き | **`textual-plotext`** 採用．軸 / 目盛 / リサイズが標準で揃ってる．狭幅端末は S-6 sparkline に自動 fallback |
| **Q-11 (新規)** | `Pictures/` の参考スクショ取扱い．gitignore で個人 aide-memoire のままか，private gist 等に逃がすか | 個人 aide-memoire のまま gitignore．設計書本文に文章で記述．スクショは再配布想定外 |
