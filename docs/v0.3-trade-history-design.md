...
> **Status**: Draft, revision 2 (incorporating 書記長 review round 1)
...
| S-6 | Inline sparkline graphs in TUI (Unicode block characters `▁▂▃▄▅▆▇█`) for per-good amount-over-time and per-route balance trend. Uses Textual's `Sparkline` widget. |
...
- Single accent for warnings/negative balance (terminal-red); selection highlight in terminal-default reverse video.
- Monospace by definition (terminal). No emojis, no decorative glyphs, no ASCII-art logos.
...
`Last seen` column shows the relative time since the most recent trade for that
row, derived from event timestamps (see §9.4). The optional `Trend` sparkline
(S-6) renders the per-row time series compressed into a fixed 8-cell bar.
...
│                                                                      │
│  ↑/↓ navigate · / filter · s sort · ←→ tab · g graph · q back        │
└──────────────────────────────────────────────────────────────────────┘
```

#### Screen 3 — Route Detail (S-2; deferred if budget tight)

When S-6 ships, the per-cycle balance also renders as a small sparkline beneath
the cargo manifest.
...
| `g` | toggle Trend sparkline column (S-6) |
| `/` | filter |
| `s` | sort cycle |
| `gg` / `G` | top / bottom |
| `j` / `k` | down / up |
...
| **Q-6 (Decided)** | Item YAML scope for v0.3.0: empty stubs vs pre-populated top-20. Bilingual (`name_en` + `name_ja`) confirmed required. | **Pre-populate top 20** in bilingual form; ship `scaffold-items` helper for the rest. |
| **Q-7 (Decided)** | TUI visual theme: bespoke "CCCP Edition" vs subdued default. | **Subdued default** (no logos, no `☭`, no bespoke palette). Single accent: terminal-red for negatives. |
...
| **Q-9 (new)** | `SECONDS_PER_TICK` for Anno 117 — confirmed 0.05 s (20 Hz) via gameplay timing? | Spike during week 1 by sampling two saves N minutes apart and diffing tick counts. |
