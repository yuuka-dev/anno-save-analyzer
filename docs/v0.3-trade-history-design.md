...
> **Status**: Draft, revision 3 (incorporating Anno 1800 reference screenshots)
...
chart). Reference screenshots live locally in `Pictures/` (gitignored under the
`Pictures/` rule for copyright reasons; do not commit).
...
|---|---|
| `q` | back / quit |
| `t` | trade statistics screen |
| `r` | route detail screen |
| `d` | snapshot diff screen (S-1) |
| `g` | toggle the right context pane (partners ↔ chart, S-7) |
| `[` / `]` | shrink / grow the focused pane |
| `/` | filter the central table |
| `s` | cycle sort column |
| `gg` / `G` | top / bottom |
| `j` / `k` | down / up |
| `h` / `l` | move focus left / right pane |
| `:export json` | command-bar export |
...
| **Q-9** | `SECONDS_PER_TICK` for Anno 117 — confirmed 0.05 s (20 Hz) via gameplay timing? | Spike during week 1 by sampling two saves N minutes apart and diffing tick counts. |
| **Q-10 (new)** | Chart library choice: `textual-plotext` (3rd-party Textual widget around `plotext`) vs. drawing the line chart by hand with Unicode block glyphs. | `textual-plotext`. It already handles axes, ticks, and resize; ship faster and degrade to sparkline (S-6) for narrow terminals. |
| **Q-11 (new)** | Reference screenshots in `Pictures/` — keep gitignored as is, or move to a private gist for record-keeping? | Keep gitignored locally. The design doc carries verbal references; the screenshots are personal aide-memoire, not redistributable. |
