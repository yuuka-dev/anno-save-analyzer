...
> **Status**: Draft (pending 書記長 review)
> **Target ship**: 2-3 weeks of part-time dev
> **Languages in this doc**: English (primary, OSS-facing) with Japanese annotations on key decisions
...
| M-3 | Resolve `GoodGuid` (int32) to a human-readable name via `data/items_anno117.yaml` and `data/items_anno1800.yaml`. | English required, Japanese optional per entry. |
...
| **i18n** | Source defaults to English. Item names accept Japanese alongside English in YAML. UI strings English first, ja translation tagged for v0.3.x. |
...
class Item(BaseModel):
    ...
    name_en: str               # always present (fallback: f"Good_{guid}")
    name_ja: str | None = None # optional, sourced from YAML
...
### 5.3 YAML item dictionary (per game)

```yaml
# data/items_anno117.yaml — bilingual, hand-curated
# Format is intentionally minimal so 書記長 can fill it in over time.
...
# Unknown GUIDs are not blocking — viewer falls back to "Good_2088" etc.
```

A small `scripts/scaffold_items_yaml.py` (delivered with v0.3) scans a save and emits a YAML stub with all GUIDs found, ready for human curation.
...
└── tui/
    ...
    │   ├── trade_summary.py
    │   └── route_detail.py  # S-2
    └── theme_cccp.py   # CCCP Edition palette
```
...
### 9.1 Textual TUI layout (CCCP Edition)

**Style guide** — *書記長's preference*:

- Palette: deep red `#8B0000`, hammer-yellow `#F4D03F`, bone-white `#F0F0E8`, charcoal base `#1A1A1A`.
- Typeface: monospace only (Cascadia Mono / JetBrains Mono / IBM Plex Mono).
- No emojis, no rounded corners, no soft shadows. Hard rules `─ │ ┌ ┐ └ ┘`.
- Footer always shows: `[anno-save-analyzer · CCCP Edition · v0.3.0]` left, save path right.
...
│                                                                 │
│ ─ Sessions ──────────────────────────────────────────────────── │
│   #0  47.9 MB   871 tags   <population: TBD>                    │
│   #1  28.0 MB   …                                               │
│                                                                 │
│ ─ Trade snapshot ────────────────────────────────────────────── │
...
#### Screen 2 — Trade Summary (item / route tabs)

```
┌─ Trade Summary ─────────────────────────────────[ items │ routes ]┐
│                                                                   │
│ Good                Bought   Sold    Net qty   Net gold           │
│ ─────────────────── ──────── ─────── ───────── ──────────         │
│ Wood (木材)              +52     -8       +44      −1,760         │
│ Bricks (煉瓦)            +14     -2       +12        −560         │
│ Wine (葡萄酒)             +0     -8        −8        +320         │
│ Good_2138                +17    -17         0           0         │
│                                                                   │
│  ↑/↓ navigate · /  filter · s sort · ←→ tab · q back              │
└───────────────────────────────────────────────────────────────────┘
```
...
│ Last visited: tick 0x080918f8 (≈14h ago)                      │
│                                                               │
│ Cargo manifest (latest cycle):                                │
│   Wood       +20 → −20 (delivered)                            │
│   Bricks     +8  → −8                                         │
│                                                               │
│ Balance over last 10 cycles: +1,240 gold                      │
└───────────────────────────────────────────────────────────────┘
```

**Hotkey philosophy**: Vim-friendly defaults pending Q-3 confirmation.

| Key | Action |
|---|---|
| `q` | back / quit |
| `t` | trade summary screen |
| `r` | route detail screen |
| `d` | snapshot diff screen (S-1) |
| `/` | filter |
| `s` | sort cycle |
| `gg` / `G` | top / bottom |
| `j` / `k` | down / up |
| `:export json` | quick command-bar export |
...
### 9.3 CCCP Edition style cues

- Title bar shows hammer-and-sickle Unicode `☭` flanked by hard rules — single accent only, never decorative.
- Critical states (negative gold, route bleeding) rendered in deep red.
- Highlights (current selection) in hammer-yellow on charcoal.
- All numerical formatting respects locale-independent comma grouping; gold values prefixed with `+` / `−` and a fixed-width `g`.
...
- Trade Summary screen with item/route tabs
- Theme `theme_cccp.py`
- `pilot`-based snapshot tests

**Exit criterion**: `anno-save-analyzer tui sample_anno117.a8s` boots, navigation works, exports trigger.

### Week 3 — Method B + polish
...
If week 3 slips, ship v0.3.0 with M-only and push S/N items into v0.3.1.
...
| **Q-3** | TUI hotkey conflicts with 書記長's Vim habits? Confirm `j/k`, `gg/G`, `/`, `:` mapping is correct. | Adopt Vim defaults unless 書記長 vetoes. |
| **Q-4** | Default snapshot interval for Method B documentation? **5 / 15 / 60 minutes**? | 15 min for active sessions; 60 min for casual play. Document both. |
| **Q-5** | Export format priority: **JSON > HTML > CSV > Markdown** OK? | Yes — JSON is the integration surface, HTML for shareables, CSV for spreadsheets, Markdown for forum posts. |
| **Q-6** | Item YAML scope for v0.3.0: ship empty stubs and let users curate, or pre-populate the ~20 most common Anno 117 goods? | Pre-populate top 20 for dogfooding; ship `scaffold-items` for the rest. |
| **Q-7** | Does CCCP Edition show the hammer-and-sickle `☭` glyph by default, or is that an opt-in flag? | Default ON for 書記長; CLI `--no-symbols` for screenshots / public demos. |
| **Q-8** | Snapshot-diff inferred events: include in the default ledger output or hide unless `--include inferred`? | Include with a clear `source: diff` flag; let downstream filter. |
...
| CCCP Edition | The TUI visual theme: deep red, hammer-yellow, monospace, no decorations. |
