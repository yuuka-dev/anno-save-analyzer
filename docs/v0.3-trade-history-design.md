...
> **Status**: Draft, revision 4 (Q-3〜Q-11 finalized; nano-flavored hotkeys)
...
| **Q-6 (Decided)** | Item YAML scope and language. | **Bilingual (`name_en` + `name_ja`) mandatory.** Pre-populate top 20 goods; ship `scaffold-items` helper for the rest. |
...
| ID | Work item | When |
|---|---|---|
| W-1 | Confirm `<TradedGoods>` parent (`<RouteMap>` vs `<ActiveTradeHandler>` vs other) via 10-minute hex-dump on `sample_anno117.a8s` Session 0. | Just before Week 1 implementation starts. |
